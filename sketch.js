...
function draw() {
  ...
  board.forEach((row, y) => {
    row.forEach((e, x) => {
      if ((x == 0 && y == 0) || (x == 9 && y == 7)) {
        let o;
        if (peice && tempOrient != null)
          o = x == peice[0] && y == peice[1] ? tempOrient : e;
        else o = e;
        push();
        ...
        if (tempOrient != null) t = (x == peice[0] && y == peice[1]) * 1;
          ...
          if (tempLoc.length == 2) t = (x == peice[0] && y == peice[1]) * 1;
            ...
            t =
              ((x == tempLoc[0] && y == tempLoc[1]) ||
                (x == peice[0] && y == peice[1])) * 1;
          }
        } else t = 0;
        t = map(t, 0, 1, 255, 150);
        translate(x * scl + scl / 2, y * scl + scl / 2);
  ...
  switch (keyCode) {
    ...
    case 38:
      if (peice) {
        let e = board[peice[1]][peice[0]];
        if (
          (peice[1] == 0 && peice[0] == 0 && turnRed) ||
          (peice[0] == 9 && peice[1] == 7 && !turnRed)
        ) {
...
function canOrient() {
  ...
  switch (e[0]) {
    case "d":
      //deflector
      return (
        abs(tempOrient - int(e[2])) == 1 || abs(tempOrient - int(e[2])) == 3
      );
    case "D":
      //defender
      switch (e[2]) {
...
function move() {
  ...
  if (board[y][x] != "") {
    let x_ = abs(x - peice[0]),
      y_ = abs(y - peice[1]),
      t =
        ((e[1] == "r") ^ (def_brd[y][x] == 2) || def_brd[y][x] == 0) &&
        !(turnRed ^ (e[1] == "r")) &&
        (board[y][x][0] == "D" || board[y][x][0] == "d");
    if (
      e[0] == "s" &&
